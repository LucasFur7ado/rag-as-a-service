...
/**
 * API key primitives (Feature 4): generation, hashing, and the KV auth-cache
 * shape. No plaintext key is ever persisted — only its SHA-256 hash. Lookup on
 * the auth path is by hash (constant work: one indexed read), never by
 * iterating and comparing stored keys.
 *
...
/** Value cached in KV under `apikey:{hash}` for fast auth-path lookups. */
...
/** KV key namespacing so API-key entries don't collide with other KV usage. */
...
/** Serialize a D1 row to the public {@link ApiKey} shape (no key material). */
