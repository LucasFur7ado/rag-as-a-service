...
import { MAX_UPSERT_BATCH_SIZE, MAX_UPSERT_REQUEST_BYTES } from "../config";
import { pineconeConfig } from "../env";
import { PermanentError } from "../lib/errors";

/**
 * Vector store seam, implemented for Pinecone — the one piece of the stack
 * that survived the move off Cloudflare unchanged.
 *
 * Talks to the Pinecone data plane over plain `fetch` (REST) instead of the
 * `@pinecone-database/pinecone` SDK: it keeps the function bundle lean, avoids
 ...
 * swappable — route handlers and the ingestion pipeline depend only on
 * `VectorStore`.
 */
...
// --- Naming conventions -----------------------------------------------------
// Centralized so ingestion, deletion, and retrieval agree on them.
...
export class PineconeVectorStore implements VectorStore {
  ...
  constructor() {
    // `pineconeConfig()` throws a MissingEnvError for an unset or placeholder
    // host, so an unconfigured deploy fails with an actionable message rather
    // than an opaque DNS error after burning the whole retry budget.
  ...
  private async request<T = unknown>(
    ...
    try {
      ...
      // Transient (retryable) — a wrong host is normally caught by the
      // placeholder check in pineconeConfig().
      throw new Error(
