 ...
 * separate from the route handlers on purpose: the handlers in
 * `src/app/api/**` do their own validation and return bespoke error messages,
 * so a validating router that intercepted them would change response bodies.
 * Registering paths instead documents the real contract without touching a
 * single line of request handling.
 *
 ...
 *
 * Paths below are relative to the API base (`/api`), which the spec carries in
 * its `servers` entries — see openApiServers() in src/server/config.ts.
 */
...
/**
 * Register every operation on an OpenAPI registry. Called by
 * {@link buildOpenApiDocument} (./document.ts), which both the live
 * `/api/v1/openapi.{json,yaml}` routes and the build-time generator use.
 */
export function registerOpenApi(r: OpenAPIRegistry): void {
  ...
  r.registerPath({
    ...
    description:
      "Delete the collection, its documents' metadata + stored files, and its vector namespace. Irreversible.",
    security: SEC_ANY,
    request: { params: idParam("col_9f8b2a1c") },
    responses: {
  ...
  r.registerPath({
    ...
    description: "Delete a document's vectors, stored file, and metadata row. Irreversible.",
    security: SEC_ANY,
    request: { params: idParam("doc_4c1e77a0") },
    responses: {
      204: { description: "Deleted." },
